...
export default {
  ...
  methods: {
    async addAppt(newAppt) {
      const res = await fetch("https://my-json-server.typicode.com/olatunji-20/appt-tracker/tasks", {
        method: "POST",
        headers: {
    ...
    async deleteAppt(id) {
      if (confirm("are you sure you want to delete this appointment?")) {
        const res = await fetch(`https://my-json-server.typicode.com/olatunji-20/appt-tracker/tasks/${id}`, {
          method: "DELETE",
          headers: {
            "Content-type": "application/json",
          },
